...
def addLeaves(tree,fileName):
    ...
    for i in range(events):
        ...
        for jet in range(2):
         ...
         isNaN = False
         if rt.TMath.IsNaN(tree.j_n2_b1_mmdt[jet]) or rt.TMath.IsNaN(tree.j_n2_b1[jet]) or rt.TMath.IsNaN(tree.j_tau32_b2_mmdt[jet]) or rt.TMath.IsNaN(tree.j_tau32_b2[jet]):
            isNaN = True
         if isNaN: continue
         ...
         for branch in tree.GetListOfBranches():
            ...
            if tree.GetBranchStatus(branch.GetName()):
                ...
                if hasattr(obj, "__getitem__"):
                    #print(branch.GetName(),"getitem",obj[jet],jet)
                    setattr(s1, branch.GetName(), obj[jet] )
                else:
